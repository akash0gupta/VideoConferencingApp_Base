      ...
      int pageSize = 20)
        {
            try
            {
                ...
                // For the first page, try to get from cache
                if (pageNumber == 1)
                {
                    var cacheKey = ContactCacheKey.SearchResults(currentUserId, normalizedQuery);
                    var cachedResults = await _cache.GetAsync<PagedList<UserSearchDto>>(cacheKey);
                ...
                var result = new PagedList<UserSearchDto>(userDtos, pageNumber, pageSize, totalCount);
        ...
        public async Task<IEnumerable<UserQuickSearchDto>> QuickSearchAsync(long currentUserId, string query, int limit = 10)
        {
            try
            {
                ...
                var normalizedQuery = query.ToLower().Trim();
                var cacheKey = ContactCacheKey.QuickSearch(currentUserId, normalizedQuery);

                // Try to get from cache first
                var cachedResults = await _cache.GetAsync<List<UserQuickSearchDto>>(cacheKey);
                if (cachedResults != null)
                    return cachedResults;
          ...
          int pageSize = 20)
        {
            try
            {
                var cacheKey = ContactCacheKey.UserContacts(userId);

                // Check if we can use cache for this request
                if (pageNumber == 1 && pageSize == 20)
                {
                    // Only cache the first page with default page size
                    var cachedContacts = await _cache.GetAsync<List<ContactDto>>(cacheKey);
                    if (cachedContacts != null)
                    {
                        var ctotalCount = cachedContacts.Count;
                        var pagedContacts = cachedContacts
                            .Skip((pageNumber - 1) * pageSize)
                            .Take(pageSize)
                            .ToList();

                        return new PagedList<ContactDto>(pagedContacts, pageNumber, pageSize, ctotalCount);
                    }
                }
                ...
                // Cache the result if it's the first page with default page size
                if (pageNumber == 1 && pageSize == 20)
                {
                    await _cache.SetAsync(cacheKey, contactDtos);
                }
        ...
        /// </summary>
        public async Task<ContactDto> GetContactByIdAsync(long contactId, long currentUserId)
        {
            try
            {
                var cacheKey = ContactCacheKey.Contact(currentUserId, contactId);

                var contactQuery = _contactRepository.Table
                    .Where(c => c.Id == contactId &&
                               (c.RequesterId == currentUserId || c.AddresseeId == currentUserId) &&
                               (c.RequesterId == contactId || c.AddresseeId == contactId) &&
                               c.Status == ContactStatus.Accepted &&
                               c.IsActive &&
                               !c.IsDeleted);
                ...
                var userIds = new[] { contact.RequesterId, contact.AddresseeId };
                var users = await _userRepository.FindAsync(u => userIds.Contains(u.Id));
                var userDict = users.ToDictionary(u => u.Id);

                var requester = userDict.GetValueOrDefault(contact.RequesterId);
                var addressee = userDict.GetValueOrDefault(contact.AddresseeId);

                return MapToContactDto(contact, requester, addressee, currentUserId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting contact {ContactId} for user {UserId}", contactId, currentUserId);
                throw;
            }
        }
        ...
        public async Task<IEnumerable<ContactRequestDto>> GetPendingRequestsAsync(long userId, RequestDirection direction = RequestDirection.Both)
        {
            try
            {
                CacheKey cacheKey = direction switch
                {
                    RequestDirection.Sent => ContactCacheKey.SentRequests(userId),
                    RequestDirection.Received => ContactCacheKey.ReceivedRequests(userId),
                    _ => ContactCacheKey.PendingRequests(userId)
                };

                // Try to get from cache first
                var cachedRequests = await _cache.GetAsync<List<ContactRequestDto>>(cacheKey);
                ...
                var result = requests.Select(r => MapToContactRequestDto(r, userDict, userId)).ToList();
        ...
        public async Task<IEnumerable<BlockedUserDto>> GetBlockedUsersAsync(long userId)
        {
            try
            {
                var cacheKey = ContactCacheKey.BlockedUsers(userId);

                // Try to get from cache first
                var cachedBlockedUsers = await _cache.GetAsync<List<BlockedUserDto>>(cacheKey);
                ...
                var result = blockedContacts.Select(c =>
                {
        ...
        private async Task InvalidateContactsCacheAsync(params long[] userIds)
        {
            foreach (var userId in userIds)
            {
                await _cache.RemoveAsync(ContactCacheKey.UserContacts(userId));
                await _cache.RemoveAsync(ContactCacheKey.PendingRequests(userId));
                await _cache.RemoveAsync(ContactCacheKey.BlockedUsers(userId));
            }
        }
