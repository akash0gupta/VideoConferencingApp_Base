...
namespace VideoConferencingApp.Infrastructure.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        private const int EMAIL_VERIFICATION_TOKEN_EXPIRY_DAYS = 7;
        ...
        public async Task<AuthenticationResultDto> RegisterAsync(RegisterRequestDto request)
        {
            ...
            try
            {
                ...
                var emailVerificationExpiry = DateTime.UtcNow.AddDays(EMAIL_VERIFICATION_TOKEN_EXPIRY_DAYS);
        ...
        public async Task<bool> RequestPasswordResetAsync(PasswordResetRequestDto request)
        {
            try
            {
                ...
                var resetTokenExpiry = DateTime.UtcNow.AddHours(PASSWORD_RESET_TOKEN_EXPIRY_HOURS);
        ...
        public async Task<bool> ResendVerificationEmailAsync(ResendVerificationDto request)
        {
            try
            {
                ...
                var tokenExpiry = DateTime.UtcNow.AddDays(EMAIL_VERIFICATION_TOKEN_EXPIRY_DAYS);
                ...
                TemplateData = new Dictionary<string, string>
        {
            ...
            { "ExpiryHours", EMAIL_VERIFICATION_TOKEN_EXPIRY_DAYS.ToString() }
        }
            });
                ...
                TemplateData = new Dictionary<string, string>
        {
            ...
            { "DisplayName", user.DisplayName },
            { "ResetUrl", resetUrl },
            { "ExpiryHours", PASSWORD_RESET_TOKEN_EXPIRY_HOURS.ToString() }
        }
            });
        ...
        private async Task<string> GenerateTwoFactorCodeAsync(User user)
        {
            try
            {
                ...
                var cacheKey = new CacheKey($"2fa_code_{user.Id}");
                await _cache.SetAsync(cacheKey, formattedCode, TimeSpan.FromMinutes(5));
        ...
        /// </summary>
        private async Task ValidateRegistrationAsync(RegisterRequestDto request)
        {
        ...
        {
            var cacheKey = new CacheKey($"rate_limit_{key}_{period.TotalMinutes}");
            ...
            await _cache.SetAsync(cacheKey, count + 1, period);
        }
